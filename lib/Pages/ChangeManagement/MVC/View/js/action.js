...
// Mitarbeiter zu Angebot
function setMaAngebot(mid,bid,location)
{
    "use strict";
    $.post("/changeManagement/setMaAngebot", {
        bid: ""+bid+"",
        mid: ""+mid+"",
        location: ""+location+""
    }, function(resp) {
        $('#ma'+mid+'').fadeOut(800);
        dspMaAngebot(location,bid);
    });
}
// Suche
function suche(parameter) // Löscht eine Zeile aus der Tabelle Versuchsbau
{
    "use strict";
    const str = parameter;
    if(str.length > 2) {
        document.getElementById('ergebnis').classList.remove('dspnone');
        $.post("/changeManagement/suche", {
            parameter: "" + parameter + ""
        }, function (text) {
            $('#ergebnis').html(text);
        })
    } else {
        document.getElementById('ergebnis').classList.add('dspnone');
    }
}
function dspHinweis(parameter)
{
    "use strict";
    const str = parameter;
    if(str.length > 2) {
        document.getElementById('hinweisSuche').classList.add('dspnone');
    } else {
        document.getElementById('hinweisSuche').classList.remove('dspnone');
    }
}
...
{
    $('#fl'+id+'').fadeOut(800);
    $.post("/changeManagement/deleteFile", { id: "" + id + "" });
}
// Berechtigung Angebote setzen
function setAccess(part,location,bid,mid)
{
    $.post("/changeManagement/setAccess", {
        part: "" + part + "",
        location: "" + location + "",
        bid: "" + bid + "",
        mid: "" + mid + ""
